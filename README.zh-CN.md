# MPE2PDF — Markdown 转 PDF

**[English README](https://github.com/ZanytriumPH/MPE2PDF/blob/main/README.md)**

将当前 Markdown 文件导出为 PDF，渲染样式借鉴
[Markdown Preview Enhanced](https://github.com/shd101wyy/markdown-preview-enhanced)（MPE）的排版风格，
提供熟悉的阅读体验。
...
- **完整公式支持**：`$..$` 行内公式、`$$..$$` 块级公式（MathJax 3，扩展全部内置、离线可用；定界符与公式间的空格/换行均可容忍）
- **图表支持**：Mermaid 图（客户端渲染，跟随明暗主题）、PlantUML 图（Java 渲染，需配置）
- **中文友好**：标题锚点保留中文、中文/空格路径自动编码
- **图片居中**：独占段落的图片（含链接包裹）自动居中，与 Mermaid/PlantUML 图一致
...
- 编辑器右键（Markdown 文件）或资源管理器右键（.md 文件）→「MPE2PDF: Export to PDF」
- 命令面板：`MPE2PDF: Export to PDF`
...
| `mpe2pdf.headingRule` | `true` | 是否显示主标题与二级标题下方的分隔线 |
...
| `mpe2pdf.rootDir` | `""` | 根路径图片（`/assets/...` 等）映射目录（空 = 源文件目录） |
